...
##Next steps: change backgorund, axis, color scheme of box plots 
...
p2 <- dat %>% 
  ...
  ggtitle("Boxplots of Life Expectancy Per State") + 
  xlab("State") + ylab("Life Expectancy (years)")

p2

##Next steps: change background, axis, color scheme of box plots 
...
p5 <- dat %>% select(county, state, Life.Expectancy) %>% arrange(Life.Expectancy)%>% 
          filter(Life.Expectancy <= 70.14277)

#creating a line plot of descending order of life expectancy
p6 <- p5 %>% ggplot(aes(x = reorder(county, -Life.Expectancy), y = Life.Expectancy, group = 1)) +
            geom_point() +
            geom_line() + 
            theme_bw() + 
...
p6 + scale_x_discrete("Cut Type", labels = c("Oglala Lakota" = "Oglala Lakota (SD)", "Buffalo" = " Buffalo (SD)", "Corson" = "Corson (SD)", "Mellette" = " Mellette (SD)","Todd" = " Todd (SD)", "Roosevelt" = "Roosevelt (MT)", "Dewey" = "Dewey (SD)", 
...
p7 <- dat %>% ggplot(aes(x = Life.Expectancy)) +
              geom_histogram(binwidth = 0.2, color = "black", fill = "light blue") + 
              theme_bw()+ 
              ggtitle("Histogram of Life Expectancy in the United States") +
              theme(plot.title = element_text(hjust = 0.5)) +
              xlab("Life Expectancy") 

p7 + geom_vline(aes(xintercept = mean(Life.Expectancy, na.rm = TRUE)), col='dark red',size=1)
...
p8 <- dat %>% ggplot(aes(x = `Drug.Overdose.Mortality.Rate`)) +
              geom_histogram(binwidth = 0.75, color = "black", fill = "gray") + 
              theme_bw()+ 
              ggtitle("Histogram of Drug Overdose Mortality Rate per 100,000 population in the U.S.") +
              theme(plot.title = element_text(hjust = 0.5)) +
              xlab("Drug Overdose Mortality Rate (per 100,000 population)") 
p8 + geom_vline(aes(xintercept = median(`Drug.Overdose.Mortality.Rate`, na.rm = TRUE)), col='dark red',size=1) 
...
##adding a trend line to show relationship between drug over dose mortality and life expectancy
