...
# Youth activities = Disconnected.Youth 
...
# random forest life exp
roc_rf <- roc(test_set$DEATH_EVENT, preds_rf6)

# confusion matrix
CFrf2 <- confusionMatrix(data = as.factor(rf_res2), reference = as.factor(test_set2$overdose), positive='1')
CFrf2
...
## Logistic regression
fit2 <-  glm(overdose ~ `%.Food.Insecure`+	`%.Uninsured`+	`%.Some.College`+	`%.Unemployed`+	Median.Household.Income+ Violent.Crime.Rate+ `%.Insufficient.Sleep`, `%.Severe.Housing.Problems`+ Life.Expectancy, data= train_set2, family="binomial")
...
## Naive Bayes
set.seed(1)
nb_fit2 <-  naiveBayes(overdose ~ `%.Food.Insecure`+	`%.Uninsured`+	`%.Some.College`+	`%.Unemployed`+	Median.Household.Income+ Violent.Crime.Rate+ `%.Insufficient.Sleep`+ `%.Severe.Housing.Problems`+ Life.Expectancy, data = train_set2)
...
library(rpart)
set.seed(1)
train_set2 <- train_set2  %>% dplyr::select(., overdose, `%.Food.Insecure`,	`%.Uninsured`,	`%.Some.College`,	`%.Unemployed`,	Median.Household.Income, Violent.Crime.Rate, `%.Insufficient.Sleep`, `%.Severe.Housing.Problems`, Life.Expectancy)
...
ggplot2::ggplot(dat_imp) +
  ...
  scale_fill_grey() +
  theme_bw() + 
  ggtitle("Variable Importance: Life Expectancy Prediction (Decision Tree)")
...
ggplot2::ggplot(dat_imp) +
  ...
  scale_fill_grey() +
  theme_bw() + 
  ggtitle("Variable Importance: Overdose Mortality Prediction (Decision Tree)")
