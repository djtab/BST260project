...
p <- dat %>% 
  ...
  theme_update(plot.title = element_text(hjust = 0.75)) + 
...
p2 <- dat %>% 
  mutate(state = reorder(state, Life.Expectancy, FUN = median)) %>%
...
p7 <- dat %>% 
  ...
  theme(axis.text.y = element_text(size = 2)) + ##need to adjust the size of text 
...
```{r}
dat %>% select(county, state, Drug.Overdose.Mortality.Rate) %>% arrange(desc(Drug.Overdose.Mortality.Rate))

p8 <- dat %>% select(county, state, Drug.Overdose.Mortality.Rate) %>% arrange(Drug.Overdose.Mortality.Rate)%>% 
          filter(Drug.Overdose.Mortality.Rate >= 61.50097)
...
```{r}
####Food Insecurity
dat %>% select(county, state, `%.Food.Insecure`) %>% arrange(desc(`%.Food.Insecure`))
...
```{r}
####%Housing Problems 
dat %>% select(county, state, `%.Severe.Housing.Problems`) %>% arrange(desc(`%.Severe.Housing.Problems`))
